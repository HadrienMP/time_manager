...
class Auth extends CI_Controller
{
	...
	function index()
	{
		if ($message = $this->session->flashdata('message')) {
		    $button = $this->session->flashdata('button');
			$this->my_display('auth/general_message', array('message' => $message, 'button' => $button));
		} else {
			redirect('/auth/login/');
		}
	}
	...
	function login()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->my_display('auth/login_form', $data);
		}
	}
	...
	function register()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->my_display('auth/register_form', $data);
		}
	}
	...
	function send_again()
	{
		if (!$this->tank_auth->is_logged_in(FALSE)) {							// not logged in or activated
			...
			$this->my_display('auth/send_again_form', $data);
		}
	}
	...
	function forgot_password()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->my_display('auth/forgot_password_form', $data);
		}
	}
	...
	function reset_password()
	{
		...
		$this->form_validation->set_rules('new_password', $this->lang->line('New Password'), 'trim|required|xss_clean|min_length['.$this->config->item('password_min_length', 'tank_auth').']|max_length['.$this->config->item('password_max_length', 'tank_auth').']|alpha_dash');
		$this->form_validation->set_rules('confirm_new_password', $this->lang->line('Confirm new Password'), 'trim|required|xss_clean|matches[new_password]');
		...
		$this->my_display('auth/reset_password_form', $data);
	}
	...
	function change_password()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->form_validation->set_rules('old_password', $this->lang->line('Old Password'), 'trim|required|xss_clean');
			$this->form_validation->set_rules('new_password', $this->lang->line('New Password'), 'trim|required|xss_clean|min_length['.$this->config->item('password_min_length', 'tank_auth').']|max_length['.$this->config->item('password_max_length', 'tank_auth').']|alpha_dash');
			$this->form_validation->set_rules('confirm_new_password', $this->lang->line('Confirm new Password'), 'trim|required|xss_clean|matches[new_password]');
			...
			$this->my_display('auth/change_password_form', $data);
		}
	}
	...
	function change_email()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->my_display('auth/change_email_form', $data);
		}
	}
	...
	function unregister()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->my_display('auth/unregister_form', $data);
		}
	}
	 ...
	 * @return	void
	 */
	function _show_message($message, $button = NULL)
	{
		$this->session->set_flashdata('message', $message);
		$this->session->set_flashdata('button', $button);
		redirect('/auth/');
	}
