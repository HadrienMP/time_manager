...
class Time_manager {
	...
	public function calculate_stats($user_id) {
		...
		foreach ( array_keys ( $time_spent ) as $period ) {
			...
			$stats ['periods'] [$period] ['overtime_t'] = calculate_overtime ( $stats ['periods'] [$period] ['time_spent_t'] + $stats ['time_left_t'], $working_time, $stats ['periods'] [$period] ['days_worked'] );
	...
	public function update_checks($checks, $checks_to_add, $ids_to_delete, $user_id) {
		...
		$this->ci->checks->update_checks ( form_to_db_checks ( $checks ), prepare_checks_to_add_for_db ( $checks_to_add ), $ids_to_delete, $user_id );
	}
	...
	public function all_pages_action($user_id) {
		...
		return array (
				'is_user_checked_in' => $last_check['check_in'] ? TRUE : FALSE,
